...
The renders pipeline has been **exercised end-to-end against the deployed production backend**, including on a repo other than `lgty-frontend` — `trulayer/kindscan-frontend`'s `visual-review-capture.yml` runs it on every push and pull request, and a real before/after gallery (with resolving artifact URLs) has been confirmed in a posted brief. Two separate production runs — one on `lgty-frontend`, one on a customer-shaped second repo with a different framework — have gone through `POST /v1/renders` and `POST /v1/renders/complete` and produced a real comparison. If you hit a mismatch, that is a bug worth an issue.
